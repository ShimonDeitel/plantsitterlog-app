import XCTest
@testable import PlantSitterLog

@MainActor
final class PlantSitterLogTests: XCTestCase {
    var store: Store!
    ...
    func testAddIncreasesCount() {
        ...
        store.add(PlantSitterLogItem(name: "Test", detail: "Detail", extra: 0, date: Date()))
        XCTAssertEqual(store.items.count, before + 1)
    }

    func testDeleteRemovesItem() {
        let item = PlantSitterLogItem(name: "ToDelete", detail: "Detail", extra: 0, date: Date())
        store.add(item)
        store.delete(item)
    ...
    func testFreeLimitBlocksAdditionalItems() {
        for i in 0..<Store.freeLimit {
            store.add(PlantSitterLogItem(name: "Item \(i)", detail: "d", extra: 0, date: Date()))
        }
    ...
    func testUpdateModifiesExistingItem() {
        let item = PlantSitterLogItem(name: "Original", detail: "Detail", extra: 0, date: Date())
        store.add(item)
        var updated = item
